...
({ __typeof__ (a) _a = (a); \
  ...
  void rangeQuery(struct nodo* albero, int dir, float* q, params* input, float** h, int qi, int l, float val, float* puntoRegione,float puntoRegioneFiglio, int n_padding,int k_padding );
    ...
    if(X != NULL){
      ...
      for (i = 0; i <padding(n); i++) {
        for(j =0; j<padding(k); j++){
          ...
          //salta di n un numero di volte pari a k
          if(i>=n || j>=k){
            break;
          }
  ...
  */
  ...
  void pca(params* input) {
    ...
    input->medieDS = _mm_malloc(sizeof(float)*k_padding, 16);
    ...
    for(int j=0; j<input->h; j++){
      ...
      do{
        ...
        //calcoliamo tPrimo=uT*u perchè u è cambiato
        tPrimo=0;
  ...
  */
  ...
  void espandi(params* input){
    ...
    int * array= _mm_malloc(input->capacitaQA*2*sizeof(int), 16);
  ...
  void range_query_pca(params* input){
    ...
    float* puntoRegione = _mm_malloc(k_padding*sizeof(float),16);
    ...
    //trova hmax e hmin
    float elem=0; 
    for(int i=0; i<input->n; i++){ //8000 volte
      for(int j=0; j<input->k; j++){ //128 volte
    ...
    // float * point= _mm_malloc(sizeof(float)*input->k, 16);
    ...
    for(int qi = 0; qi<input->nq; qi++){
      ...
      if(distance<=raggio){
        ...
        input->QA[input->indiceQA]=(albero->row/(k_padding))-(k_padding-input->k);
        input->indiceQA++;
        input->nQA++;
      }
      ...
      float hAllocato = puntoRegione[0]; 


      if(albero->sx)
      //con 0 la funzione capisce che sto arrivando da sx
      rangeQuery(albero->sx, 0, queryPoint, input, h, qi, 1, hFiglio,puntoRegione,hAllocato,n_padding,k_padding); //indiceQA va messa in param


      if(albero->dx)
      //con 1 la funzione capisce che sto arrivando da dx
      rangeQuery(albero->dx, 1, queryPoint, input, h, qi, 1, hFiglio,puntoRegione,hAllocato,n_padding,k_padding);
  ...
  //dir = 0 se figlio sx, dir = 1 se figlio dx
  void rangeQuery(struct nodo* albero, int dir, float* queryPoint, params* input, float** h, int qi, int l, float valPadre, float* puntoRegione,float puntoRegionePadre, int n_padding,int k_padding){
    ...
    float prevVal;
    float prevValAllocato;
    float* data = input->ds;
    //calcola dimensione
    //prendi il valore
    int c = (l-1)%input->k;

   prevValAllocato=puntoRegione[c]; 
   puntoRegione[c]=puntoRegionePadre; 
    ...
    if(distance>input->r){
      ...
      h[dir][c] = prevVal; 
      puntoRegione[c] = prevValAllocato; 
      return; 
    }
    ...
    float hAllocato = puntoRegione[l%input->k];



    if(albero->sx){
      rangeQuery(albero->sx, 0, queryPoint, input, h, qi, l+1, hFiglio,puntoRegione,hAllocato,n_padding,k_padding);
    }
    ...
    if(albero->dx){
      rangeQuery(albero->dx, 1, queryPoint, input, h, qi, l+1, hFiglio,puntoRegione,hAllocato,n_padding,k_padding);
